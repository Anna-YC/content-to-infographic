...
所有样式支持自定义落款。落款品牌按以下优先级决定：

1. 用户在本次请求中明确指定的落款
2. 环境变量 `INFOGRAPHIC_FOOTER_BRAND`
3. 不设置落款（默认无落款）

最终落款格式建议为：`By <品牌名> ♦ YYYY.MM.DD`。未指定时默认不加落款。
...
| 4 | 宫崎骏风格 | 可自定义 | 通用/备用/治愈系 |
| 5 | 复古海报风 | 可自定义 | 通用/备用/怀旧 |
| 6 | 赛博霓虹风 | 可自定义 | 通用/备用/未来感 |
| 7 | 自然森系风 | 可自定义 | 通用/备用/环保 |
| 8 | 杂志编辑风 | 可自定义 | 通用/备用/高端 |
| 9 | 蛋仔风格 | 可自定义 | 通用/备用/可爱 |
| 10 | 新海诚风格 | 可自定义 | 通用/备用/风景 |
| 11 | 素描风格 | 可自定义 | 通用/备用/艺术 |
| 12 | 极简商务风 | 可自定义 | 通用/备用/商务 |
| 13 | 水墨国风 | 可自定义 | 通用/备用/国潮 |
| 14 | 像素游戏风 | 可自定义 | 通用/备用/怀旧游戏 |
| 15 | 波普艺术风 | 可自定义 | 通用/备用/潮流 |
| 16 | 科幻太空风 | 可自定义 | 通用/备用/科幻 |
| 17 | 蒸汽朋克风 | 可自定义 | 通用/备用/机械 |
| 18 | 孟菲斯设计风 | 可自定义 | 通用/备用/创意 |
| 19 | 日式浮世绘风 | 可自定义 | 通用/备用/和风 |
| 20 | 3D等距风 | 可自定义 | 通用/备用/立体 |

> 样式4-20为**备用样式**。用户未指定样式时，随机选用其中之一。
...
**落款规则：**
- 所有样式均支持自定义落款（如 `Corner small: 'By Your Brand ♦ 2026.05.25'`）。若用户未指定，优先使用环境变量 `INFOGRAPHIC_FOOTER_BRAND`；均未设置时默认不加落款。
...
3. 按模板构建Prompt（落款规则：所有样式支持自定义落款，默认不加）
4. **直接运行脚本**：
   ```bash
   # macOS / Linux
...
### 关键约束（必须遵守）
1. **不生成二维码**：参见主文档"关键约束"
2. **未指定样式**：随机选4-20，默认不加落款，且**不告诉用户选了哪个样式**
